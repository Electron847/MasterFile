...
object fp1 {
  ...
  //val numberList = (0 to 100).toList
  ...
  val factTest : List[Int] = {
    ...
    val ft = List(fact(1),fact(2),fact(3),fact(4),fact(5))
    ft
  }
  ...
  // loop.
  // web resource used for logic on recursive summation
  // https://alvinalexander.com/scala/how-to-extract-sequence-elements-collection-scala-cookbook
  ...
  //resource to understand tail recursion with a sum function
  ...
  def maxTailAux (accumulator : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
   // case Nil => accumulator
   // case y :: ys => maxTailAux(accumulator, xs)
    -1
  }
  ...
  def otpu (start : Int, end : Int) : List[Int] = {
    val emptyList = List()
    val upToList = List[Int](start)

    if(start < end)
      return emptyList
    else
      upToList ::: otpu(start-1,end)
  }
}