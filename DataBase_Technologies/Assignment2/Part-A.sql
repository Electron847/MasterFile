...
--Q1 Select the names of the products with a price less than or equal to $200
select name from products where price <= 200;

--Q2 Select all the products with a price between $60 and $120
select name from products where price > 60 and price <120;

--Q3 Select the name and price in cents (i.e., the price is in dollars).
select name, price*100 from products;

--Q4 Select the product name, price, and manufacturer name of all the products.
select p.name, p.price, m.name from products p join manufacturers m on p.manufacturer = m.code;

--Q5 Select all manufactures who currently do not have any listed products.
select name from manufacturers minus 
(select m.name from manufacturers m, products p where p.manufacturer = m.code);

--Q6 Select the name of each manufacturer along with the name and price of its most expensive product.
...
--Q7Select the names and average prices of manufacturer whose products have an average price larger than or equal to $150
