...
--4
select * from trips right join byplane on trips.tid = byplane.tid 
where fare > 1000 and class = 'Business'
--5
select travelmode, tripstate
from trips
where travelmode = 'Car' or travelmode = 'Train';


--6
select tid, mileage
from bycar bc outer join bycar bb on bc.mileage = bb.mileage;

--7
Select unique trainspeed From
byTrain
;
...
--9
Select tripstate, travelmode FROM
trips
GROUP By tripstate, travelmode;
--10