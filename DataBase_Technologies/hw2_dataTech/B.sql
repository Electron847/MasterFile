...
(DEPARTMENT D FULL OUTER JOIN EMPLOYEE E ON D.Mgr_ssn = E.ssn)
WHERE E.Dno IS NULL;
...
--5
SELECT Dname, Count(E.Ssn) NumberEmployees, Count(dPen.Essn) DependentNumber FROM
(DEPARTMENT I FULL OUTER JOIN EMPLOYEE E ON I.Dnumber=E.Dno)
FULL OUTER JOIN Dependent dPen ON dPen.Essn = E.Ssn
GROUP BY Dname
HAVING COUNT(E.Ssn) > 0;
--6
Select e.Salary, e.Fname, e.Lname From
EMPLOYEE E 
Where Salary BETWEEN (Select Max(Salary - 20000) from EMPLOYEE) AND (Select Max(Salary) from EMPLOYEE)
;
