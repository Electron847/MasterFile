...
--1
Select name, cuisine FROM
RESTAURANT R
WHERE R.cuisine = 'Indian';
--2
Select name, ra.stars From
(RESTAURANT R FULL OUTER JOIN RATING Ra ON R.rID = Ra.rID)
WHERE ra.stars >= 4
ORDER BY ra.stars ASC;
--3
Select name, ra.stars From
(RESTAURANT R FULL OUTER JOIN RATING Ra ON R.rID = Ra.rID)
WHERE ra.stars IS NULL;
--4
Select name, ra.ratingdate FROM
(REVIEWER Rev FULL OUTER JOIN RATING Ra ON Rev.vID = Ra.vId)
WHERE ra.ratingdate IS NULL;
--5
Select unique r.name, rev.name FROM
(REVIEWER Rev FULL OUTER JOIN RATING Ra ON Rev.vID = Ra.vID)
FULL OUTER JOIN RESTAURANT R ON R.rID = ra.rid;
