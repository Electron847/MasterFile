    ...
    public void MultiSend () { // Multicast some data to each of the processes.
        ...
        try {
            for (int i = 0; i < numProcesses; i++) {// Send our key to all servers.
                ...
                toServer = new PrintStream(sock.getOutputStream());
                toServer.println("FakeKeyProcess " + generateKeyPair(PID).getPublic());
                toServer.flush();
                sock.close();
            }
            ...
            String BlockRecord = "(Block#" + Integer.toString(((bc.PID + 1) * 10) + 3) + " from P" + bc.PID + ")";
            ...
            for (int i = 0; i < numProcesses; i++) {// Send a sample unverified block B to each server
                sock = new Socket(serverName, Ports.UnverifiedBlockServerPortBase + (i * 1000));
                toServer = new PrintStream(sock.getOutputStream());
                toServer.println("New Block Record Arrived: " + BlockRecord);
                //toServer.println(BlockRecord.toCharArray());
                toServer.flush();
                sock.close();
            }
        } catch (Exception x) {
