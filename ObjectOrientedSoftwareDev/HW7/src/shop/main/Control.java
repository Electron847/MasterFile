...
class Control {
  ...
  private Menu_UI[] _menus;
  private int _state;
  private FormBuilder _getVideoForm;
  ...
  Control(Inventory inventory, UI ui) {
    ...
    _stringTest = new UIFormTest() {
      ...
      FormBuilder f = FormBuilder.getUIFormBuilder();
    f.add("Title", _stringTest);
    f.add("Year", yearTest);
    f.add("Director", _stringTest);
    Form _getVideoForm = f.toUIForm("Enter Video");
  }
  ...
  private void addSTART(int stateNum) {
    MenuBuilder m = MenuBuilder.getUIMenuBuilder();//changed -Brad
    ...
    m.add("Add/Remove copies of a video",
      new UIMenuAction() {
        public void run() {
          String[] result1 = null;
		try {
			result1 = _ui.processForm(_getVideoForm);
		} catch (Exception e) {
			e.printStackTrace();
		}
          ...
          String[] result2 = null;
		try {
			result2 = _ui.processForm(f.toUIForm(""));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//    UIMenuBuilder m = new UIMenuBuilder();
    MenuBuilder m = MenuBuilder.getUIMenuBuilder();
