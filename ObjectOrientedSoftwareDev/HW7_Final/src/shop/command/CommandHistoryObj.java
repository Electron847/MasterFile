...
final class CommandHistoryObj implements CommandHistory {
  ...
  RerunnableCommand _undoCmd = new RerunnableCommand () {
      public boolean run () {
        ...
        UndoableCommand hldpop = null;
        if (result) {
        	hldpop = _undoStack.pop();
        	   System.out.println("hldpop; "+ hldpop);
        	 _redoStack.push(hldpop);
      	         	
        }
        if(hldpop!= null) hldpop.undo();  
        return result;
      }
    };
  RerunnableCommand _redoCmd = new RerunnableCommand () {
      public boolean run () {
        ...
        UndoableCommand hldpop = null;
        if (result) {
        	hldpop = _redoStack.pop();
        	 _undoStack.push(hldpop);
        }
        if(hldpop != null) hldpop.redo();   
        return result;
      }
    };
