...
class KVClient (stores: Seq[ActorRef]) {
  ...
  val store = stores.toList

  //println("inside KVClient unpacking 'store' info: " + store)
  ...
  def hashForKey(nodeID: Int, cellSeq: Int): BigInt = {
    ...
    //println(label)
    //println("hey from hashcode " + (md.digest(label.getBytes)).hashCode() + " within the groupID " + nodeID + " at " + cellSeq.self ) //generates specific hashkey for each object under each different client
    BigInt(1, digest)
  }
  ...
  private def route(key: BigInt): ActorRef = {
    //println("inside route " + stores((key % stores.length).toInt))  //stores groupID 128 bit space
