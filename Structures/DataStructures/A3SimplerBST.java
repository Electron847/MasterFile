...
public class A3SimplerBST<K extends Comparable<? super K>, V> {
	...
	private int leavesNum = 0;
	private int longestPathFromRoot = 0;
	...
	public int longestPathLength() {
		this.longestPathFromRoot = 0;
		longestPathLengthHelper(root, 0);
		return this.longestPathFromRoot;
	}
	
	private void longestPathLengthHelper(Node node, int Currlayer) {
		if(node == null) return;
		if(node.left == null && node.right == null) {
			this.longestPathFromRoot = Math.max(Currlayer, this.longestPathFromRoot);
		}
		longestPathLengthHelper(node.left, Currlayer+1);
		longestPathLengthHelper(node.right, Currlayer+1);
	}
	
	public int leafCount() {
		this.leavesNum = 0;
		leafCountHelper(root);
		return this.leavesNum;
	}
	
	private void leafCountHelper(Node node) {
		if(node == null) return;
		if(node.left == null && node.right == null) {
			this.leavesNum++;
		}
		leafCountHelper(node.left);
		leafCountHelper(node.right);
	}
}
