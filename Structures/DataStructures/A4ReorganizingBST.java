...
public class A4ReorganizingBST<K extends Comparable<? super K>, V> {
	...
	private Iterable<K> rebuildTree(){ 			// non-recursive private method
		countChanges = 0; 						// Reset the change counter to 0
		Queue<K> qKeys = new Queue<K>();		// Build an array of the keys (use the keys() method; build a queue, change to an array?)
		qKeys = keys();							// keys return the Queue -> add keys to the Queue, then transfer to an AList
		ArrayList<K> aKeys = new ArrayList<K>();
		int len = qKeys.size();
		for (<K> i : qKeys) {					// Iterate through the Queue
			aKeys.add(i);						// Add value to an array list
		}
		Arrays.sort(aKeys);						// Sort the array
		listInMedianOrder(aKeys);				// Call the next method described below to get a queue of the keys in median order
		Node <K,V> root = new Node<>();	// Declare a Node variable; this will be the root of the new and balanced tree 
		this.root = root;
		for (int key: aKeys){					// Iterate through the queue returned by that method and call the public put, 
			root = put(root);					// 											passing it the root of the new tree
			//Change the root instance variable to refer to this new root
		}	
	}
	private Node<K,V> rebuildTree(Node<K,V> x, Queue<K> q){ //recursive private method - helper
		//		x.finished_left = rebuildtree(arr[0:arr.length()/2], left);
		//		x.left = finished_left; 
		//		x.right = finished_right;

		rebuildTree(x.left, q);
		q.enqueue(x.key);
		rebuildTree(x.right, q);
		return x;
	}
	 ...
	 *  Rebuilds existing Tree from scratch to rebalance 
	 *****************************************************************************/
	private Iterable<K> listInMedianOrder (Node<K,V> x, ArrayList<K> arr){ //use sorted ArrayList of keys as a parameter
		//		1st inserted value - median of an array
		//		2nd median of the array to the left (leftMedian)
		//		3rd median of the array to the right (rightMedian)
		//		recursion?
		//		return;
		int len = arr.size();
		...
		while (!qLeftIndex.isEmpty()){					//	while the left index queue is not empty:
			...
			if (left <= right) {						//		if left <= right then
				...
				qMedian.enqueue(medianIndex);			// 			enqueue the key in the array at the median index onto the median queue
				qLeftIndex.enqueue();					//	    	enqueue left and the median index minus one onto the left index queue
				qRightIndex.enqueue();					//		    enqueue right and the median index plus one onto the right index queue		
			}
