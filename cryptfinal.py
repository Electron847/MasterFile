...
# P + Q = n+1 - phi_n
# P - Q = sqrt( (n+1 - phi_n)**2 - 4n)

n = 898607526590969848863184322603417866026220164569611859928589
phi_n = 898607526590969848863184322601520436048324820954773803576156

def find_PandQ(n,phi_n):
    P_plus_Q = n+1 - phi_n
    P_minus_Q = math.sqrt( (n+1 - phi_n)**2 - 4*n)
    ...
    print("Q is: "+str(Q))
    print("typed n: "+str(n))
    print("calculated n = P*Q: "+str(bigN))
    
find_PandQ(n,phi_n)
#Assignment 4 Problem Number 3


#Instructions to run:  Compile and Run this program.
#Then call: driver().
#This will prompt for plainText and key then print the ciphertext.
...
def driver():
    ...
    for key in range(0,512):
        ciphertextBeforeTheShift = encrypt(plainText,key)
        shift(ciphertextBeforeTheShift)

def shift(ciphertextBefore):
    for x in range(1,13):
        ciphertextBefore = ciphertextBefore>>1
        cipherBin = '{0:012b}'.format(ciphertextBefore)
        print(str(x)+":   CipherText After Shift = " + str(int(ciphertextBefore)) +" ("+ str(cipherBin)+")\n")
...
def encrypt(plainText, key):
    ...
    ciphertextBeforeShift = int(nextSubKeyPairs3[0]+nextSubKeyPairs3[1],2)
    print("CipherText Before Shift = " + str(int(nextSubKeyPairs3[0]+nextSubKeyPairs3[1],2)) +" ("+ str(nextSubKeyPairs3[0]+nextSubKeyPairs3[1])+")")
    return ciphertextBeforeShift




nB = 2968567020983224981
eB = 1790217391
iDnumber = 1917410
ILheigh = 1235

message = iDnumber*ILheigh
cMessage = (message**eB) % nB

print("cMessage is: ", cMessage)
